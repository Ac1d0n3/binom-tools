...
final class LinkInventoryScanner
{
    ...
    public function scan(): array
    {
        ...
        foreach ($this->scanPhpConfigValue(config('vendor-resources', []), 'config/vendor-resources.php') as $hit) {
            $found[] = $hit;
        }
        foreach ($this->scanPhpConfigValue(config('compliance', []), 'config/compliance.php') as $hit) {
            $found[] = $hit;
        }
