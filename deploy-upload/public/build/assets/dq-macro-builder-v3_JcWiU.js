import{p as e}from"./pii-meta-D_9Jsj4F.js";function t(t){let n=e(t.selectedWarehouse),r=n.regexMatch(`{{ column_ref }}`,`{{ pattern }}`).replace(/\{\{ column_ref \}\}/g,`{{ column_ref }}`).replace(/'\{\{ pattern \}\}'/g,`'{{ pattern }}'`);return`{# macros/dq_governance.sql — Step 1: DQ Macro Generator #}
{# Warehouse: ${n.label} | Copy into macros/ in your dbt project #}
...
{% macro dq_record_result(model_name, column_name, rule_id, rule_type, severity, status, failed_rows, total_rows, metric_value) %}
  ...
  {% do run_query(sql) %}
{% endmacro %}
`}function n(){return`{# tests/generic/dq_rule.sql — Step 1: DQ Macro Generator #}
...
{% test dq_rule(model, column_name, rule) %}
  ...
  {% endif %}
{% endtest %}
`}function r(t){let n=e(t.selectedWarehouse);return`# dbt Data Quality — SETUP

Generated for **${n.label}**. Settings are configured in binom-tools and shared via localStorage across all DQ setup steps.
...
| \`example_dq_schema.yml\` | 2 — DQ Rules Generator | \`models/schema/${t.modelName}.yml\` |
...
Model: **${t.modelName}** | Warehouse: **${n.label}**
`}export{n,r,t};