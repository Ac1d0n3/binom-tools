...
@section('admin_content')
    <div class="tools-content tools-content--wide sp-app admin-hub">
        <h1 class="tools-page-title">{{ $isNew ? 'New plan template' : 'Edit: '.$slug }}</h1>
        <form method="post" action="{{ $isNew ? locale_route('admin.plan-templates.store') : locale_route('admin.plan-templates.update', ['slug' => $slug]) }}" class="admin-hub__editor">
            @csrf
        ...
        @unless ($isNew)
            <form method="post" action="{{ locale_route('admin.plan-templates.destroy', ['slug' => $slug]) }}" onsubmit="return confirm('Delete both locales?');" style="margin-top:1rem">
                @csrf
                @method('DELETE')
                <button type="submit" class="tools-btn tools-btn--danger">Delete template</button>
            </form>
        @endunless
    </div>
@endsection
