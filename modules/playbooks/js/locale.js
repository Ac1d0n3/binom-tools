 ...
 * @returns {number}
 */
function readMinutesForSeriesCard(element, locale) {
...
export function refreshSeriesCardMeta(locale = getLocale()) {
    document.querySelectorAll('[data-playbook-series-card-meta]').forEach((element) => {
        ...
        const readMinutes = readMinutesForSeriesCard(element, locale);
...
export function initPlaybookLocale() {
    if (
        !document.querySelector('[data-playbook-locale-panel], [data-playbook-nav-title], [data-playbook-index-card], [data-playbook-series-card-meta]')
    ) {
        return;
    }
