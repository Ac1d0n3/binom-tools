---
type: sprint-plan
title: Zweites Quartal — Option 2 dbt statt QVD-Script-Logik
slug: data-reporting-q2-dbt-qvd-qlik
description: Q2-Option 2 für die gewachsene Fabric/QVD/Qlik-Landschaft: fachliche Logik aus QVD-/Qlik-Skripten herausziehen, als dbt-Modelle mit Delta Loads, SCD und Tests neu aufbauen und erste App-Items anbinden.
duration: 13
unit: week
...
roadmap_phase: 2
roadmap_option: Weg 3 Option 2 Fabric + dbt ersetzt Qlik-Scripte
roadmap_follows:
...
Dreizehn Wochen Q2-Umsetzung mit zwei Personen: vorhandene App-Logik aus bestehenden Qlik-Apps, QVD-Generatoren, SUBs, DELTAs und SCD-Logiken herausziehen, ein belastbares Star-Schema mit Delta Loads und Slowly Changing Dimensions bauen und den ersten nutzbaren Anschluss in die Qlik-App liefern.

```sprint
id: week-01
number: 1
title: Bestehende App-Logik extrahieren
goal: dbt als neue Transformationsschicht für Qlik: Bestehende App-Logik extrahieren.
...
tasks:
  - id: extract-existing-logic
    ...
    assigneeId: null
    helpText: |
      Vorhandene Logik, Kennzahlen, Filter und Sonderskripte aus bestehenden Qlik-Apps, QVD-Generatoren, SUBs, DELTAs und SCD-Logiken erfassen. Ziel ist nicht, alles zu kopieren, sondern die fachliche Logik vom Tool-Code zu trennen.
...
goal: dbt als neue Transformationsschicht für Qlik: Star-Schema und App-Scope schneiden.
dependsOn: week-01
...
goal: dbt als neue Transformationsschicht für Qlik: Quellen auf Zielmodell mappen.
dependsOn: week-02
...
goal: dbt als neue Transformationsschicht für Qlik: Delta Loads entwerfen.
dependsOn: week-03
...
goal: dbt als neue Transformationsschicht für Qlik: Slowly Changing Dimensions entwerfen.
dependsOn: week-04
...
goal: dbt als neue Transformationsschicht für Qlik: Staging- und Basis-Modelle bauen.
dependsOn: week-05
...
goal: dbt als neue Transformationsschicht für Qlik: Star-Schema bauen.
dependsOn: week-06
...
goal: dbt als neue Transformationsschicht für Qlik: DQ- und Reconciliation-Tests umsetzen.
dependsOn: week-07
...
goal: dbt als neue Transformationsschicht für Qlik: Gegen alte App-Werte validieren.
dependsOn: week-08
...
goal: dbt als neue Transformationsschicht für Qlik: Report-App anbinden.
dependsOn: week-09
...
tasks:
  - id: connect-report-app
    ...
    assigneeId: null
    helpText: |
      Das neue Modell in die erste Qlik-App laden, Datenverbindung, Semantik und Berechtigungen fuer den Pilotumfang herstellen.
      Achte darauf, vorhandene App-Logik fachlich zu verstehen, bevor sie technisch neu gebaut wird.
    linkedStories: building-from-scratch, dq-test-kpis
    helpLinks:
      - label: Qlik-App
...
goal: dbt als neue Transformationsschicht für Qlik: Erste App-Items anlegen.
dependsOn: week-10
...
goal: dbt als neue Transformationsschicht für Qlik: Betrieb, Runbook und Ownership haerten.
dependsOn: week-11
...
goal: dbt als neue Transformationsschicht für Qlik: Q2 abschliessen und Q3-Backlog schneiden.
dependsOn: week-12
