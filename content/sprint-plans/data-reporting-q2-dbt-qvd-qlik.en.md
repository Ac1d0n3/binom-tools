---
type: sprint-plan
title: Second Quarter — Option 2 dbt instead of QVD script logic
slug: data-reporting-q2-dbt-qvd-qlik
description: Q2 option 2 for the grown Fabric/QVD/Qlik landscape: extract business logic from QVD/Qlik scripts, rebuild it as dbt models with delta loads, SCD, and tests, and connect first app items.
duration: 13
unit: week
...
roadmap_phase: 2
roadmap_option: Path 3 Option 2 Fabric + dbt replaces Qlik scripts
roadmap_follows:
...
goal: dbt as the new transformation layer for Qlik: Extract existing app logic.
...
goal: dbt as the new transformation layer for Qlik: Slice star schema and app scope.
dependsOn: week-01
...
goal: dbt as the new transformation layer for Qlik: Map sources to target model.
dependsOn: week-02
...
goal: dbt as the new transformation layer for Qlik: Design delta loads.
dependsOn: week-03
...
goal: dbt as the new transformation layer for Qlik: Design slowly changing dimensions.
dependsOn: week-04
...
goal: dbt as the new transformation layer for Qlik: Build staging and base models.
dependsOn: week-05
...
goal: dbt as the new transformation layer for Qlik: Build star schema.
dependsOn: week-06
...
goal: dbt as the new transformation layer for Qlik: Implement DQ and reconciliation tests.
dependsOn: week-07
...
goal: dbt as the new transformation layer for Qlik: Validate against old app values.
dependsOn: week-08
...
goal: dbt as the new transformation layer for Qlik: Connect report app.
dependsOn: week-09
...
goal: dbt as the new transformation layer for Qlik: Create first app items.
dependsOn: week-10
...
goal: dbt as the new transformation layer for Qlik: Harden operations, runbook, and ownership.
dependsOn: week-11
...
goal: dbt as the new transformation layer for Qlik: Close Q2 and slice Q3 backlog.
dependsOn: week-12
