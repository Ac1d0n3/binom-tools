---
type: sprint-plan
title: Second Quarter — Snowflake / dbt / Qlik follow-up
slug: data-reporting-q2-snowflake-dbt-qlik
description: Q2 follow-up for Fivetran/Snowflake/dbt/Qlik: extract prioritized app logic from existing reports, build the star schema, implement delta loads and SCD, and connect first Qlik app items.
duration: 13
unit: week
...
Thirteen weeks of Q2 implementation with two people: extract existing app logic from existing Qlik apps and QVD/script logic, build a reliable star schema with delta loads and slowly changing dimensions, and deliver the first usable connection into the Qlik app.

```sprint
id: week-01
number: 1
title: Extract existing app logic
goal: Snowflake/dbt/Qlik: Extract existing app logic.
...
tasks:
  - id: extract-existing-logic
    ...
    assigneeId: null
    helpText: |
      Capture current logic, metrics, filters, and special scripts from existing Qlik apps and QVD/script logic. The goal is not copying everything, but separating business logic from tool code.
...
goal: Snowflake/dbt/Qlik: Slice star schema and app scope.
dependsOn: week-01
...
goal: Snowflake/dbt/Qlik: Map sources to target model.
dependsOn: week-02
...
goal: Snowflake/dbt/Qlik: Design delta loads.
dependsOn: week-03
...
goal: Snowflake/dbt/Qlik: Design slowly changing dimensions.
dependsOn: week-04
...
goal: Snowflake/dbt/Qlik: Build staging and base models.
dependsOn: week-05
...
goal: Snowflake/dbt/Qlik: Build star schema.
dependsOn: week-06
...
goal: Snowflake/dbt/Qlik: Implement DQ and reconciliation tests.
dependsOn: week-07
...
goal: Snowflake/dbt/Qlik: Validate against old app values.
dependsOn: week-08
...
goal: Snowflake/dbt/Qlik: Connect report app.
dependsOn: week-09
...
goal: Snowflake/dbt/Qlik: Create first app items.
dependsOn: week-10
...
goal: Snowflake/dbt/Qlik: Harden operations, runbook, and ownership.
dependsOn: week-11
...
goal: Snowflake/dbt/Qlik: Close Q2 and slice Q3 backlog.
dependsOn: week-12
