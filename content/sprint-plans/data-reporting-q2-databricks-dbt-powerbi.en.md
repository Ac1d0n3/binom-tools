---
type: sprint-plan
title: Second Quarter — Databricks Lakehouse / dbt / Power BI follow-up
slug: data-reporting-q2-databricks-dbt-powerbi
description: Q2 follow-up for Databricks Lakehouse/dbt/Power BI: extract prioritized app logic from existing reports, build the star schema, implement delta loads and SCD, and connect first Power BI app items.
duration: 13
unit: week
...
roadmap_phase: 2
roadmap_option: Path 4 Databricks Lakehouse / dbt / Power BI
roadmap_follows:
  - data-reporting-fq-databricks-dbt-powerbi
...
Thirteen weeks of Q2 implementation with two people: extract existing app logic from existing Power BI reports, datasets, and upstream logic, build a reliable star schema on Delta tables with incremental loads and slowly changing dimensions, and deliver the first usable connection into the Power BI app.

```sprint
id: week-01
number: 1
title: Extract existing app logic
goal: Databricks Lakehouse/dbt/Power BI: Extract existing app logic.
...
tasks:
  - id: extract-existing-logic
    ...
    assigneeId: null
    helpText: |
      Capture current logic, metrics, filters, and special scripts from existing Power BI reports, datasets, and upstream logic. The goal is not copying everything, but separating business logic from tool code.
...
goal: Databricks Lakehouse/dbt/Power BI: Slice star schema and app scope.
dependsOn: week-01
...
goal: Databricks Lakehouse/dbt/Power BI: Map sources to target model.
dependsOn: week-02
...
goal: Databricks Lakehouse/dbt/Power BI: Design delta loads.
dependsOn: week-03
...
goal: Databricks Lakehouse/dbt/Power BI: Design slowly changing dimensions.
dependsOn: week-04
...
goal: Databricks Lakehouse/dbt/Power BI: Build staging and base models.
dependsOn: week-05
...
goal: Databricks Lakehouse/dbt/Power BI: Build star schema.
dependsOn: week-06
...
goal: Databricks Lakehouse/dbt/Power BI: Implement DQ and reconciliation tests.
dependsOn: week-07
...
goal: Databricks Lakehouse/dbt/Power BI: Validate against old app values.
dependsOn: week-08
...
goal: Databricks Lakehouse/dbt/Power BI: Connect report app.
dependsOn: week-09
...
tasks:
  - id: connect-report-app
    ...
    assigneeId: null
    helpText: |
      Load the new model into the first Power BI app, wiring data connection, semantics, and permissions for the pilot scope.
      Watch that existing app logic is understood functionally before it is rebuilt technically.
    linkedStories: building-from-scratch, dq-test-kpis
    helpLinks:
      - label: Power BI app
...
goal: Databricks Lakehouse/dbt/Power BI: Create first app items.
dependsOn: week-10
...
goal: Databricks Lakehouse/dbt/Power BI: Harden operations, runbook, and ownership.
dependsOn: week-11
...
goal: Databricks Lakehouse/dbt/Power BI: Close Q2 and slice Q3 backlog.
dependsOn: week-12
